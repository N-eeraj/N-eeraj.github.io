...
import { POLL_QUERY_KEY } from "@blogs/constants//productEngineeringMatrix"
...
export function useFetch() {
  const {
    ...
    queryFn: async () => {
      const { data } = await request(`${WEBSITE}/api/blogs/${BLOG_SLUG.PRODUCT_ENGINEERING_MATRIX}`)
      return data
    },
  })
...
export function useSubmit(userVote?: Option) {
  ...
  const {
    ...
    mutationFn: async () => {
      await request(`${WEBSITE}/api/blogs/${BLOG_SLUG.PRODUCT_ENGINEERING_MATRIX}`, {
